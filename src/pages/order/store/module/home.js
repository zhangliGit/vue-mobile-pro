...
const localData = window.localStorage.getItem('order') || JSON.stringify({})
...
const home = {
  ...
  mutations: {
    ...
    updateData(state, { key, data, isLocal = true }) {
      if (isLocal) {
        const quiData = JSON.parse(localStorage.getItem('order') || JSON.stringify({}))
        quiData[key] = data
        window.localStorage.setItem('order', JSON.stringify(quiData))
      }
      state[key] = data
    }
  }
